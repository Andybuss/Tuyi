...
public class AsynImageLoader {
    ...
    public static final int ForHead =0;
    public static final int ForNormal=1;
    // 缓存下载过的图片的Map
...
private static AsynImageLoader instance;
     ...
     */
    public void showImageAsyn(ImageView imageView, String url, int resId,int w,int h,int style) {
        imageView.setTag(url);
        Bitmap bitmap =loadImageAsyn(url, getImageCallback(imageView, resId,w,h,style));
        if (bitmap == null) {
            ...
            imageView.setImageBitmap(PhotoUtil.zoomBitmap(bitmap,w,h));
        }
    }
     ...
     * @return
     */
    private ImageCallback getImageCallback(final ImageView imageView, final int resId,final int w,final int h,final int style) {
        return new ImageCallback() {

            @Override
            public void loadImage(String path, Bitmap bitmap) {
                if (path.equals(imageView.getTag().toString())) {
                    if(style==AsynImageLoader.ForHead)
